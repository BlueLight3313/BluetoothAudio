...
static void usage(){
  ...
  LOGI("  --audio linuxraw|opensles");
  LOGI("  --pcm-capture <dev>   (linuxraw only)");
  LOGI("  --pcm-playback <dev>  (linuxraw only)");
}

int main(int argc,char** argv){
  const char* tcp="127.0.0.1:3333";
  const char* ux="/dev/socket/bt_chatd";
  const char* audio="linuxraw";
  const char* pcm_cap="/dev/snd/pcmC0D0c";
  const char* pcm_pb="/dev/snd/pcmC0D0p";
  int use_tcp=1;
  int adapter_index=0;
  ...
  for(int i=1;i<argc;i++){
    ...
    else if(strcmp(argv[i],"--pcm-capture")==0 && i+1<argc){ pcm_cap=argv[++i]; }
    else if(strcmp(argv[i],"--pcm-playback")==0 && i+1<argc){ pcm_pb=argv[++i]; }
